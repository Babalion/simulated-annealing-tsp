...
void TSPInstance::createNewYork(int n) {
    float latticeConst = 1000.0f / static_cast<float>(n);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; ++j) {
...
void Optimizer::optimize(const TSPInstance &instance, std::vector<int> &result) const {
    ...
    Config config(lengthOfMemory);

    if (static_cast<int>(result.size()) ==
        n) {// if the given result is already a valid tour //TODO test this case better
        config.state = result;
    ...
    for (config.outer = 0; config.outer < outerLoops; config.outer++) {
        ...
        for (config.inner = 0; config.inner < innerLoops; config.inner++) {
            if (config.lengthOfMemory > 0 && config.inner%50==0) {//only save every 50 point bec. of autocorrelation
                config.lastEnergies.push_back(config.energy);
            }
            if (config.lastEnergies.size() > config.lengthOfMemory) {
                config.lastEnergies.pop_front();
            }
